...
public class SignUpFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            // 1. Check all user typing errors
            if (!ValidationUitl.isValidEmail(email)) {
                foundErr = true;
                errors.setEmailInvalid("Email is invalid");
            } // end if username length is error
            if (password.trim().length() < 6 || password.trim().length() > 30) {
                foundErr = true;
                errors.setPasswordLengthErr("Password must be from 6 to 30 chars");
            } else if (!confirm.trim().equals(password.trim())) {
                foundErr = true;
                errors.setConfirmNotMatched("Confirm must match password");
            } // end if password length is error
            if (fullName.trim().length() < 2 || fullName.trim().length() > 50) {
                foundErr = true;
                errors.setFullNameLengthErr("Full name must be from 2 to 50 chars");
            } // end if full name length is error
